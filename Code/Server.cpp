...
void *communicate (void *message)
{
	...
	if (recv(client_sock, client_message, sizeof(client_message),0) < 0)
	{
		...
		Logger::Error(_FILE,LINE_,"SERVER ERROR");
...
int main(void)
{
	...
	cout<<"Bind Done\n";
	Logger::Info(_FILE,LINE_,"SERVER and CLIENT Bind is done" );
	...
	Logger::Info(_FILE,LINE_,"SERVER Listening for Incoming Connections.... ");
	...
	{		
		...
		Logger::Info(_FILE,LINE_,"SERVER Client Connected with IP: %s and Port No: ");
	...
	close(socket_desc);
        ...
        //Opening Votes textfile	
	fstream file;
        file.open("Votes.txt",ios::in|ios::binary);
        Votes();
        file.close();
        
	//Opening Voters List textfile
	file.open("Voters_List.txt",ios::in|ios::binary);
        Votes();
        file.close();
        
	//Opening Candidates List textfile
	file.open("Candidates_List.txt",ios::in|ios::binary);
        Votes();
        file.close();
